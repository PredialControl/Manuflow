...
interface EditContractUserFormProps {
    contractId: string;
    user: {
        id: string;
        name: string;
...
export function EditContractUserForm({ contractId, user }: EditContractUserFormProps) {
    ...
    async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            const res = await fetch(`/api/contracts/${contractId}/users/${user.id}`, {
                method: "PATCH",
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-black tracking-tighter uppercase italic">Editar Colaborador</h1>
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">{user.name}</p>
                </div>
            </div>
                    ...
                    <form onSubmit={onSubmit} className="space-y-6">
                        ...
                        {selectedRole === "TECHNICIAN" && (
                            <div className="space-y-2 animate-in slide-in-from-top-2">
                                ...
                                <select
                                    id="category"
                                    name="category"
                                    defaultValue={user.category || "GERAL"}
                                    className="flex h-12 w-full rounded-xl border border-border/40 bg-muted/30 px-4 py-2 text-sm font-bold focus:bg-background transition-all cursor-pointer"
                                >
                                    {categories.map((cat) => (
                        ...
                        <div className="pt-4">
                            <Button
                                ...
                                disabled={loading}
                            >
                                {loading ? <Loader2 className="h-5 w-5 animate-spin mr-2" /> : <Save className="h-5 w-5 mr-3" />}
                                Salvar Alterações
                            </Button>
                        </div>
                    </form>
